...
@Component({
  selector: 'app-reporte-mes-zona',
  templateUrl: './reporte-mes-zona.component.html',
  styleUrls: ['./reporte-mes-zona.component.css']
})
export class ReporteMesZonaComponent implements OnInit {
  config = {};
  ...
  ngOnInit(): void {
    const subUpdate = this.update.subscribe(year => {
      const subReporte = this._reportesService.getCasosZonaByMes(year)
        .subscribe(resp => {
    ...
    this.config = Object.assign({}, {
      ...
      options: {
        ...
        plugins: {
          ...
          title: {
            display: true,
            text: 'Numero de Atenciones por Zona'
          },
          subtitle: {
            display: true,
