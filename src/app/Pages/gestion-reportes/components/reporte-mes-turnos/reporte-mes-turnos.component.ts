...
@Component({
  selector: 'app-reporte-mes-turnos',
  templateUrl: './reporte-mes-turnos.component.html',
  styleUrls: ['./reporte-mes-turnos.component.css']
})
export class ReporteMesTurnosComponent implements OnInit {
  ...
  ngOnInit(): void {
    const subUpdate = this.update.subscribe(year => {
      const subReporte = this._reportesService.getCasosTurnosByMes(year)
        .subscribe(resp => {
  ...
  private processResp(resp: ReporteMesTurno[]) {
    const labels = [];
    const data = [];
