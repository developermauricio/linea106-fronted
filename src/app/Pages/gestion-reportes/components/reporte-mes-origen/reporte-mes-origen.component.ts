...
@Component({
  selector: 'app-reporte-mes-origen',
  templateUrl: './reporte-mes-origen.component.html',
  styleUrls: ['./reporte-mes-origen.component.css']
})
export class ReporteMesOrigenComponent implements OnInit {
  config = {};
  ...
  ngOnInit(): void {
    const subUpdate = this.update.subscribe(year => {
      const subReporte = this._reportesService.getCasosOrigenByMes(year)
        .subscribe(resp => {
  ...
  private processResp(resp: ReporteMesOrigen[]) {
    const labels = [];
    const data = [];
