...
@Component({
  selector: 'app-reporte-mes-ocupacion',
  templateUrl: './reporte-mes-ocupacion.component.html',
  styleUrls: ['./reporte-mes-ocupacion.component.css']
})
export class ReporteMesOcupacionComponent implements OnInit {
  config = {};
  ...
  ngOnInit(): void {
    const subUpdate = this.update.subscribe(year => {
      const subReporte = this._reportesService.getCasosOcupacionByMes(year)
        .subscribe(resp => {
    ...
    this.config = Object.assign({}, {
      ...
      options: {
        ...
        plugins: {
          ...
          title: {
            display: true,
            text: 'Numero de Atenciones por Ocupación'
          },
          subtitle: {
            display: true,
