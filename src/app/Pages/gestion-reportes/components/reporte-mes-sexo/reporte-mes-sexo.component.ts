...
@Component({
  selector: 'app-reporte-mes-sexo',
  templateUrl: './reporte-mes-sexo.component.html',
  styleUrls: ['./reporte-mes-sexo.component.css']
})
export class ReporteMesSexoComponent implements OnInit {
  ...
  ngOnInit(): void {
    const subUpdate = this.update.subscribe(year => {
      const subReporte = this._reportesService.getCasosSexoByMes(year)
        .subscribe(resp => {
    ...
    this.config = Object.assign({}, {
      ...
      options: {
        ...
        plugins: {
          ...
          title: {
            display: true,
            text: 'Numero de Atenciones por Sexo'
          },
          subtitle: {
            display: true,
